...
static int nexus_powersave_bias_setspeed(struct cpufreq_policy *policy,
...
static void nexus_powersave_bias_init_cpu(int cpu)
{
...
static void nexus_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		nexus_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_nexus Governor Tunables */
 ...
 * rate is 10 ms because the user needs immediate reaction from nexus
...
skip_this_cpu:
		...
		for_each_online_cpu(cpu) {
			...
			if (dbs_info->cur_policy) {
				/* cpu using nexus, cancel dbs timer */
				mutex_lock(&dbs_info->timer_mutex);
				dbs_timer_exit(dbs_info);

				nexus_powersave_bias_setspeed(
					dbs_info->cur_policy,
					NULL,
					input);
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of nexus, waiting for disk IO is an
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		nexus_powersave_bias_init_cpu(cpu);
		/*
		...
		if (!nexus_powersave_bias_setspeed(
...
MODULE_DESCRIPTION("'cpufreq_nexus' - A dynamic cpufreq governor for "
